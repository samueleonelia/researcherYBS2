...
RUN_VARS = {
    ...
    "PART_NOTE", "PART_ITEMS", "PARTS", "SLOT_JOB", "BASE_TIME", "POOL_LINE",
    "BASE_STORIES", "BASE_DROPPED",
...
def test_agent_files_are_generated():
    ...
    for f in sorted((ROOT / ".claude" / "agents").glob("ybs4-*.md")):
        # /ybs-shows builds its own three from its own templates.
        by = ("ybs_shows.py" if f.stem.startswith("ybs4-shows-") else "ybs_run.py")
        ...
        check(f"{f.name} says it is generated, below the frontmatter",
              f"Generated by {by} build" in after, f"line after the frontmatter: {after!r}")
