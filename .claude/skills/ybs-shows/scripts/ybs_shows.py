...
def load_settings(path: Path = None) -> dict:
    """Read the shows' two tables out of the root settings.md.

    That one file holds the article brief's and the X list's tables too. Only
    two headings belong to this skill, and every other section is skipped,
    which is why the brief and the shows may both name a number
    `agents_active_max` without clashing.

    Under `## Shows numbers` a row is `| key | value | meaning |`. Values:
    digits are ints, `50%` is the int 50, `"a", "b"` is a list of strings,
    anything else is the text as written.

    Under `## Shows models` a row is `| step | model | effort | ... |`, and
    gives two keys, `<step>_model` and `<step>_effort`, so a template can ask
    for either.
    """
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if key.lower() in ("setting", "step") or set(key) <= set("-: "):
            continue
        if section not in ("shows numbers", "shows models"):
            continue  # the other two halves' sections; their own loaders read those
        if section == "shows models":
        ...
        if re.fullmatch(r"\d+", raw):
            ...
            out[key] = raw
    if not out:
        die(f"{path} holds no shows settings (no `## Shows numbers` heading?)")
    return out
...
# The agent files under .claude/agents/ are generated from the templates in
# .claude/skills/ybs-shows/agents/. `fill`'s own regex above is uppercase-only
# and cannot see a dotted name, so `build` carries its own.
PLACEHOLDER = re.compile(r"\{\{([A-Za-z_][A-Za-z0-9_.-]*)\}\}")

GENERATED = ("<!-- Generated by ybs_shows.py build from "
             ".claude/skills/ybs-shows/agents/{name}.md.tmpl \u2014 edit the "
             "template, not this file. -->")


def namespace() -> dict:
    """Every name a template may ask for: one `settings.<key>` per row."""
    return {f"settings.{k}": str(v) for k, v in SETTINGS.items()}
...
def cmd_build(args):
    """Render .claude/agents/ybs4-shows-*.md from the templates in the skill.

    An agent file is static: Claude Code loads it as written, so the model and
    the effort a step runs at cannot be looked up at launch time. Rendering is
    how the `## Shows models` table in settings.md reaches the agents.
    """
    ...
    stale, written = [], []
    for tp in templates:
        name = tp.name[:-len(".md.tmpl")]
        text, missing = render(tp.read_text(encoding="utf-8"), ns)
        if missing:
            die(f"{tp.name} asks for {', '.join(missing)}, which nothing provides")
